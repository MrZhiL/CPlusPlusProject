...
// 常用集合算法：并集集  set_union(iteration beg1, end1, beg2, end2, dest);
// set_union返回的是迭代器，该迭代器指示的是取完交集后的容器的末尾 
// 注意：：：：：求并集的两个容器 必须是有序序列(且两个容器的序列需要为升序序列) 
...
void test01(){
	...
	// 取vec1和vec2容器的交集，存放到vec3容器中
	vector<int>::iterator vec3End = set_union(vec1.begin(), vec1.end(), vec2.begin(), vec2.end(), vec3.begin());
	
	// 遍历vec3容器中的元素，查看是否存在交集 
	// 如果使用vec3.end()标志容器末尾，则会打印容器中所有的大小，没有取到交集的元素会自动赋值为0
	// 因此需要使用set_union函数仿函的迭代器 
	...
	vec3End = set_difference(vec1.begin(), vec1.end(), vec2.begin(), vec2.end(), vec4.begin());
	cout << "两个容器差集，差集元素为： "; 
	for_each(vec4.begin(), vec3End, showPrint);
	cout << endl;	 
